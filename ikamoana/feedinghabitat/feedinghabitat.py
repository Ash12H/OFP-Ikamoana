...
class FeedingHabitat:
    ...
    def __init__(self, xml_filepath: str, root_directory: str = None, days_length_float_32: bool = True):
        """
    ...
    def controlArguments(
        self, time_start: int, time_end: int, lat_min: int, lat_max: int, lon_min: int, lon_max: int
        ...
        if time_start is not None:
            if (time_start < 0) or (time_start >= coords["time"].data.size):
                raise ValueError(
                    "time_start out of bounds. Min is %d and Max is %d" % (0, coords["time"].data.size - 1)
                )
    ...
    def _selSubMask(
        self, mask: str, lat_min: int = None, lat_max: int = None, lon_min: int = None, lon_max: int = None
    ) -> np.ndarray:
        """
    ...
    def _temperature(
        ...
        sigma_star = self._sigmaStar(
            self.data_structure.parameters_dictionary["sigma_0"], self.data_structure.parameters_dictionary["sigma_K"]
        )
        T_star = self._tStar(
        ...
        layer_buffer = []
        iterate = zip(["temperature_L1", "temperature_L2", "temperature_L3"], ["mask_L1", "mask_L2", "mask_L3"])

        for layer_name, mask_name in iterate:
            variable = self._selSubDataArray(
                self.data_structure.variables_dictionary[layer_name],
                time_start,
                time_end,
                lat_min,
                lat_max,
            ...
            layer_buffer.append(
                np.exp(
                    ((-np.power((variable - T_star), 2)) / (2.0 * (sigma_star**2))),
    ...
    def _oxygen(
        ...
        layer_buffer = []
        iterate = zip(["oxygen_L1", "oxygen_L2", "oxygen_L3"], ["mask_L1", "mask_L2", "mask_L3"])
        ...
        for layer_name, mask_name in iterate:
            ...
            if self.data_structure.partial_oxygen_time_axis:
                ...
                variable = self._selSubDataArray(variable, time_start, time_end, lat_min, lat_max, lon_min, lon_max)

            mask = self._selSubMask(mask_name, lat_min, lat_max, lon_min, lon_max)

            layer_buffer.append(np.where(mask, 1.0 / (1.0 + (np.power(gamma, (variable - o_star)))), 0.0))
    ...
    def _forage(
        ...
        # Compute Layers ######################################################
        layer_1 = np.add(
            days_length * epi, night_length * (epi + mumeso + hmlmeso), out=np.zeros_like(days_length), where=mask_L1
        )
    ...
    def _applyTopography(
        self,
        habitat: Union[xr.DataArray, xr.Dataset],
        lat_min: int = None,
        lat_max: int = None,
        lon_min: int = None,
        lon_max: int = None,
        ...
        topography = self.data_structure.topography[
            lat_min : lat_max if lat_max is None else lat_max + 1, lon_min : lon_max if lon_max is None else lon_max + 1
        ]
    ...
    def computeFeedingHabitat(
        ...
        if control_arg:
            (time_start, time_end, lat_min, lat_max, lon_min, lon_max) = self.controlArguments(
        ...
        for cohort_number in cohorts:
            ...
            fh_temperature = self._temperature(cohort_number, time_start, time_end, lat_min, lat_max, lon_min, lon_max)

            name = "Feeding_Habitat_Cohort_%d" % (cohort_number)

            result_np_array = np.where(
                mask_L1, self._scaling(np.sum(fh_forage * (fh_temperature * fh_oxygen + 1e-4), axis=0)), np.nan
            )
            ...
            result_xr_data_array = xr.DataArray(
                ...
                name=name,
                coords=dict(
                    lon=self.data_structure.coords["lon"].data[lon_min : lon_max if lon_max is None else lon_max + 1],
                    lat=self.data_structure.coords["lat"].data[lat_min : lat_max if lat_max is None else lat_max + 1],
                ...
                attrs={
                    ...
                    "Age start (days)": self.data_structure.species_dictionary["cohorts_starting_age"][cohort_number],
                    "Age end (days)": self.data_structure.species_dictionary["cohorts_final_age"][cohort_number],
                    "Length (cm)": self.data_structure.species_dictionary["cohorts_mean_length"][cohort_number],
                    "Weight (kg)": self.data_structure.species_dictionary["cohorts_mean_weight"][cohort_number],
                },
            )
    ...
    def computeEvolvingFeedingHabitat(
        ...
        if (cohort_start < 0) or (cohort_start >= cohorts_number):
            raise ValueError("cohort_start out of bounds. Min is %d and Max is %d" % (0, cohorts_number - 1))
        if (cohort_end < 0) or (cohort_end >= cohorts_number):
            raise ValueError("cohort_end out of bounds. Min is %d and Max is %d" % (0, cohorts_number - 1))
        ...
        for i in range(max_size):
            ...
            if (cohort_array[i:].size == 1) and (time_array[i:].size > 1):
                ...
                final_array.append(
                    self.computeFeedingHabitat(
                        cohorts=cohort_array[i],
                        time_start=time_array[i],
        ...
        return xr.DataArray(
            ...
            attrs=dict(
                ...
                cohort_start=cohort_axis[0],
                cohort_end=cohort_axis[-1],
    ...
    def correctEpiTempWithVld(self):
        ...
        """
        print(
            "Warning : This function (correctEpiTempWithVld) was only tested"
        ...
        dTdz = np.divide(
            2.0 * (sst.data - temperature_L1.data),
            vld.data,
    ...
    def indonesianFilter(self):
        ...
        temp1.loc[loc_4] = np.minimum(temp1.loc[loc_4], 18)
        temp2.loc[loc_4] = np.minimum(temp2.loc[loc_4], 15)
