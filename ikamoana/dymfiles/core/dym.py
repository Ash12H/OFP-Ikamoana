...
def getFileFormat(filename):
    ...
    if IdFormat in ["DYM2", "DYM3"]:
        ...
        if len(file_list) != 2:
            logging.warning("%s: expecting 2 files in archive, %d found " % (filename, len(file_list)))
            return unknown
        try:
...
class DymFile:
    ...
    def readAllData(self):
        data = np.zeros((self.header_.nLev_, self.header_.nLat_, self.header_.nLon_), np.float32)

        for i in range(self.header_.nLev_):
    ...
    def mean(self, step=None, domain=None, filename=None, header=None):
        ...
        if domain is None:
            ...
            lonmin = float(domain[0])
            lonmax = float(domain[1])
            latmin = float(domain[2])
            latmax = float(domain[3])
        ...
        if step is None:
            for k in range(self.header_.nLev_):
                ...
                data[mask == 0] = 0
                mean = np.sum(data) / n
                line = "%3d\t%s\t%f" % (k + 1, floatToStr(self.header_.zLev_[k], dateFormat="%d-%m-%Y"), mean)
                if filename is not None:
                    f.write(line + "\n")
                else:
                    print(line)
            ...
            for k in range(self.header_.nLev_):
                ...
                if int(k + 1) % step != 0:
                    ...
                    line = "%3d\t%s\t%f" % (
                        k + 1,
                        floatToStr(self.header_.zLev_[k - step / 2], dateFormat="%d-%m-%Y"),
                        stepmean / step,
                    )
                    stepmean = 0
                    if filename is not None:
    ...
    def sum(self, step=None, domain=None, filename=None, header=None):
        ...
        if step is None:
            for k in range(self.header_.nLev_):
                ...
                data[mask == 0] = 0
                sum = np.sum(data)
                line = "%3d\t%s\t%f" % (k + 1, floatToStr(self.header_.zLev_[k], dateFormat="%d-%m-%Y"), sum)
                if filename is not None:
                    f.write(line + "\n")
                else:
                    print(line)
