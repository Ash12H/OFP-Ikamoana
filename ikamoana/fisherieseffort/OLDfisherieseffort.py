...
def rescaleFisheries(
    ...
    latitude_label: str = "lat",
    longitude_label: str = "lon",
    effort_label: str = "E",
    catch_label: str = "C",
) -> Union[dict, None]:
    """
    ...
    if resolution > min_res:
        warnings.warn("For now, downscaling is not supported then we use the minimal resolution.")
    ...
    def computeSubfisheriesPositions(resolution, min_resolution, latitude, longitude):
        ...
        mat_lon = np.tile(np.linspace(lon_inf, lon_sup, nb_elem_lon) % max_longitude, nb_elem_lat)
    ...
    def computeRightPosition(row):
        return [row["sub_fishery_latitude"][row["sub_fishery"]], row["sub_fishery_longitude"][row["sub_fishery"]]]

    update_index = ["sub_fishery", "sub_fishery_latitude", "sub_fishery_longitude", effort_label, catch_label]
    ...
    for name, fishery in fisheries.items():
        if max(fishery[resolution_label].unique()) > resolution:
            ...
            sub_fishery[[latitude_label, longitude_label]] = sub_fishery.apply(
                computeRightPosition, axis=1, result_type="expand"
            )
            sub_fishery = sub_fishery.drop(columns=["sub_fishery_latitude", "sub_fishery_longitude", "sub_fishery"])
...
def groupByFisheries(
    ...
    for f_name, fishery in fisheries.items():
        f_gb = fishery.groupby(by=[fishery_label, year_label, month_label, day_label, latitude_label, longitude_label])
        f_gb = f_gb.aggregate(
            {gear_label: np.unique, resolution_label: np.unique, effort_label: np.sum, catch_label: np.sum}
        )
...
def fisheriesToDataSet(
    ...
    for f_name, fishery in fisheries.items():
        fishery_time = pd.to_datetime(
            fishery.rename(columns={year_label: "year", month_label: "month", day_label: "day"})[
                ["year", "month", "day"]
            ]
        )
        update_dict = {
        ...
        fishery_to_convert = pd.DataFrame(
            {
                "time": fishery_time,
    ...
    return (
        xr.Dataset(fisheries_effort_da_dict, attrs={"Type": effort_label}),
        xr.Dataset(fisheries_catch_da_dict, attrs={"Type": catch_label}),
    )


def _completeCoords(data_array: xr.DataArray, deltaT: int = None, space_reso: Union[float, int] = None) -> xr.DataArray:
    """
...
def normalizeCoords(
    data_set: xr.Dataset,
    deltaT: Union[int, List[int]] = None,
    ...
    for pos, f_name in enumerate(data_set):
        f_effort_dict[f_name] = _completeCoords(data_set[f_name], deltaT[pos], space_reso[pos])
...
def sumDataSet(fisheries: xr.Dataset, name: str = None) -> xr.DataArray:
    ...
    return xr.DataArray(
        ...
        name=name,
        attrs={"Type": "Sum of " + fisheries.attrs["Type"], **fisheries.attrs},
    )


def toTextFile(
    fisheries: Union[dict, pd.DataFrame],
    filepath: str = "./ouput.txt",
    fishery_label: str = "f",
    gear_label: str = "gr",
    year_label: str = "yr",
    month_label: str = "mm",
    ...
    """
    if (not isinstance(fisheries, dict)) and (not isinstance(fisheries, pd.DataFrame)):
        raise ValueError(
            "Argument fisheries must be a dict or a pandas DataFrame. Actual " f"type is {type(fisheries)}"
        )

    if isinstance(fisheries, dict):
        fisheries = pd.concat(fisheries.values())

    fisheries = fisheries.sort_values(
        [fishery_label, year_label, month_label, day_label, latitude_label, longitude_label]
    )
    fisheries = fisheries[
        [
            fishery_label,
...
def removeEmptyEntries(
    ...
    """
    if verbose:
        print("Removed %d empty entries." % (len(fishery[(fishery[catch_label] == 0) & (fishery[effort_label] == 0)])))
        print(
            "Number of empty entries per fishery :\n",
            fishery[(fishery[catch_label] == 0) & (fishery[effort_label] == 0)][[fisheries_label, catch_label]]
            .groupby([fisheries_label])
            .count()
            .rename(columns={catch_label: "empty"})
            .T,
            end="\n\n",
        )
...
def removeNoCatchEntries(
    ...
    """
    if verbose:
        print("Removed %d entries without catch." % (len(fishery[fishery[catch_label] == 0])))
        print(
            "Number of entries without catch per fishery :\n",
            fishery[fishery[catch_label] == 0][[fisheries_label, catch_label]]
            .groupby([fisheries_label])
            .count()
            .rename(columns={catch_label: "no catch"})
            .T,
        )
...
def plotByGear(
    ...
    for g, c in zip(gr, color):
        plt.plot(
            fishery[fishery[gear_label] == g][catch_label],
...
def predictEffort(
    fishery: pd.DataFrame,
    conserve_no_catch: bool = False,
    conserve_empty: bool = False,
    gear_to_choose: str = "K",
    catch_label: str = "C",
    gear_label: str = "gr",
    effort_label: str = "E",
) -> pd.DataFrame:
    ...
    """
    model = fishery[
        (fishery[gear_label] == gear_to_choose) & (fishery[effort_label] != 0) & (fishery[catch_label] != 0)
    ]

    effort_to_predict = pd.DataFrame(fishery[~fishery.index.isin(model.index)])

    no_catch_but_effort = effort_to_predict[
        (effort_to_predict[effort_label] != 0) & (effort_to_predict[catch_label] == 0)
    ]

    nothing = effort_to_predict[(effort_to_predict[effort_label] == 0) & (effort_to_predict[catch_label] == 0)]

    if effort_to_predict.size != 0:
        effort_to_predict = effort_to_predict[(effort_to_predict[catch_label] != 0)]

        regr = linear_model.LinearRegression()
        fun = regr.fit(np.array(model[catch_label])[:, np.newaxis], np.array(model[effort_label]))

        effort_to_predict.drop(effort_label, axis=1)
        effort_to_predict[effort_label] = fun.predict(np.array(effort_to_predict[catch_label])[:, np.newaxis])

    df_to_return = [model, effort_to_predict]
    if conserve_no_catch:
        df_to_return.append(no_catch_but_effort)
    if conserve_empty:
        df_to_return.append(nothing)
...
def predictEffortAllFisheries(
    fishery: dict,
    conserve_no_catch: bool = True,
    conserve_empty: bool = True,
    gear_to_choose: dict = {9: "K"},
    catch_label: str = "C",
    gear_label: str = "gr",
    effort_label: str = "E",
) -> dict:
    ...
    for key, value in fishery.items():
        if key in gear_to_choose:
            ...
            gear_list, index = np.unique(value[gear_label], return_counts=True)
            gear = gear_list[np.argmax(index)]

        new_value = predictEffort(value, conserve_no_catch, conserve_empty, gear, catch_label, gear_label, effort_label)
...
def effortByFishery(
    ...
    if remove_fisheries is not None:
        for f in remove_fisheries:
            fisheries_effort = fisheries_effort.drop(fisheries_effort[fisheries_effort[fishery_label] == f].index)
