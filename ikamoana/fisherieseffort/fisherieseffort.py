...
def readFiles(
    ...
    fishery_time = pd.to_datetime(
        fishery.rename(columns={_labels["year"]: "year", _labels["month"]: "month", _labels["day"]: "day"})[
            ["year", "month", "day"]
        ]
    )
...
def _reindexSpace(
    effort_df: pd.DataFrame, lat_coords: np.ndarray, lon_coords: np.ndarray, limit: float
) -> pd.DataFrame:
    """
    Parameters
    ----------
    ...
    effort_df[_labels["latitude"]] = effort_df[_labels["latitude"]].apply(nearestSpace, args=(lat_coords, limit))
    effort_df[_labels["longitude"]] = effort_df[_labels["longitude"]].apply(nearestSpace, args=(lon_coords, limit))

    return effort_df


def _reindexTime(effort_df: pd.DataFrame, time_coords: np.ndarray, limit: int) -> pd.DataFrame:
    """
    Parameters
    ----------
    limit : int
    ...
    effort_df["time"] = effort_df["time"].apply(nearestTime, args=(time_coords, limit))
    return effort_df
...
def selectFisheries(
    ...
    selected_fisheries: Union[str, List[str]],
) -> pd.DataFrame:
...
def rescaleFisheries(effort_df: pd.DataFrame, resolution: float = None):
    ...
    update_index = [
        "sub_fishery",
        "sub_fishery_latitude",
        "sub_fishery_longitude",
        _labels["effort"],
    ]
    ...
    for name, fishery in f_dict.items():
        if max(fishery[_labels["resolution"]].unique()) > resolution:
            ...
            sub_fishery[[_labels["latitude"], _labels["longitude"]]] = sub_fishery.apply(
...
def groupByFisheries(effort_df: pd.DataFrame, multi_index=False) -> dict:
    """
    ...
    ).aggregate({_labels["gear"]: np.unique, _labels["resolution"]: np.unique, _labels["effort"]: np.sum})
...
def fisheriesToDataSet(effort_df: pd.DataFrame) -> xr.Dataset:
    """
    ...
    for f in effort_df["f"].unique():
        ...
        tmp = tmp[["time", _labels["latitude"], _labels["longitude"], _labels["effort"]]]

        tmp = tmp.groupby(["time", _labels["latitude"], _labels["longitude"]]).sum().to_xarray()["E"]
...
def sumDataSet(fisheries: xr.Dataset, name: str = None) -> xr.DataArray:
    ...
    return xr.DataArray(
        data=sum, coords=fisheries.coords, name=name, attrs={"Type": "Sum of Effort", **fisheries.attrs}
    )
...
def removeEmptyEntries(fishery: pd.DataFrame, verbose: bool = False) -> pd.DataFrame:
    """
...
def predictEffortAllFisheries(
    ...
    for key, value in fishery.items():
        ...
        new_value = _predictEffort(
            value, conserve_no_catch, conserve_empty, gear, catch_label, gear_label, effort_label
        )
...
def effortByFishery(
    ...
    fisheries_effort = reindexCoordinates(fisheries_effort, coords, time_reso, space_reso)
