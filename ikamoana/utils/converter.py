...
def convertToField(field: Union[xr.DataArray, xr.Dataset], name=None) -> parcels.Field:
    ...
    return parcels.FieldSet.from_xarray_dataset(
        ((field.reindex(lat=field.lat[::-1])) if field.lat[0] > field.lat[-1] else field),
...
def convertToDataArray(field: parcels.Field) -> xr.DataArray:
    ...
    return xr.DataArray(data=field.data, coords={"time": time_list, "lat": field.lat, "lon": field.lon})


def convertToNauticMiles(
    field: Union[xr.DataArray, xr.Dataset], timestep: float = 1.0, square: bool = False
