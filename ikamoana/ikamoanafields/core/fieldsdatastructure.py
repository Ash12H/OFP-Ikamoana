...
from ikamoana.utils import tagReading


class IkamoanaFieldsDataStructure:
    def __init__(self, IKAMOANA_config_filepath: str, SEAPODYM_config_filepath: str = None, root_directory: str = None):
        """
        ...
        if SEAPODYM_config_filepath is None:
            SEAPODYM_config_filepath = tagReading(root, ["directories", "seapodym_file"])
    ...
    def _readMortality(self, root: ET.Element):
        iter_fisheries = root.find("mortality").find("selected_fisheries").findall("fishery")
        if len(iter_fisheries) < 1:
            raise ValueError(
                "You must specify at least one fishery in "
    ...
    def _readFisheries(self, root: ET.Element, root_directory):
        ...
        ## FISHERY FILES #################################
        directory_fishery = os.path.join(root_directory, root.find("strdir_fisheries").attrib["value"])
        files_list = os.listdir(directory_fishery)
        if files_list == []:
            raise ValueError("The directory that should contain the fisheries effort files is empty.")
        self.fishery_filepaths = [os.path.join(directory_fishery, path) for path in files_list]
        ...
        if len(list_fishery_name) != nb_fishery:
            raise ValueError(
                f"nb_fishery is {nb_fishery} but list_fishery_name contains {len(list_fishery_name)} elements."
            )

        f_param = {}
        for f in list_fishery_name:
            tmp_dict = {
                "function_type": int(root.find("s_sp_fishery").find(f).find("function_type").attrib["value"]),
                "q": float(root.find("q_sp_fishery").find(f).attrib[species_name]),
                "variable": float(root.find("s_sp_fishery").find(f).attrib[species_name]),
                "length_threshold": float(
                    root.find("s_sp_fishery").find(f).find("length_threshold").attrib[species_name]
                ),
            }

            if tmp_dict["function_type"] == 3:
                tmp_dict["right_asymptote"] = float(
                    root.find("s_sp_fishery").find(f).find("right_asymptote").attrib[species_name]
                )
            f_param[f] = tmp_dict
