...
class IkamoanaFields:
    ...
    def __init__(
        ...
        if feeding_habitat is None:
            ...
            self.feeding_habitat = latitudeDirection(feeding_habitat, south_to_north=True)
        else:
            raise TypeError(
                "feeding_habitat must be a Xarray.DataArray or None." f"Current type is : {type(feeding_habitat)}"
            )

        self.ikamoana_fields_structure = core.IkamoanaFieldsDataStructure(
            IKAMOANA_config_filepath, SEAPODYM_config_filepath, root_directory=root_directory
        )
        self.feeding_habitat_structure = FeedingHabitat(
            self.ikamoana_fields_structure.SEAPODYM_config_filepath, root_directory=root_directory
        )
    ...
    def computeFeedingHabitat(
        ...
        # returned section.
        (time_start, time_end, lat_min, lat_max, lon_min, lon_max) = self.feeding_habitat_structure.controlArguments(
            time_start, time_end, lat_min, lat_max, lon_min, lon_max
        )

        feeding_habitat = self.feeding_habitat_structure.computeFeedingHabitat(
            cohort, time_start, time_end, lat_min, lat_max, lon_min, lon_max, False
        )
        ...
        self.feeding_habitat = latitudeDirection(feeding_habitat_da, south_to_north=True)
    ...
    def computeEvolvingFeedingHabitat(
        ...
        feeding_habitat = self.feeding_habitat_structure.computeEvolvingFeedingHabitat(
            cohort_start, cohort_end, time_start, time_end, lat_min, lat_max, lon_min, lon_max, False
        )
        self.feeding_habitat = latitudeDirection(feeding_habitat, south_to_north=True)
    ...
    def current(self) -> Tuple[xr.DataArray, xr.DataArray]:
        ...
        u = fhcf.seapodymFieldConstructor(
            self.feeding_habitat_structure.data_structure.root_directory + self.ikamoana_fields_structure.u_file,
            dym_varname="u_L1",
        )
        v = fhcf.seapodymFieldConstructor(
            self.feeding_habitat_structure.data_structure.root_directory + self.ikamoana_fields_structure.v_file,
            dym_varname="v_L1",
        )
        ...
        if self.feeding_habitat is not None:
            ...
            u = u[mintime_idx : maxtime_idx + 1, minlat_idx : maxlat_idx + 1, minlon_idx : maxlon_idx + 1]
            v = v[mintime_idx : maxtime_idx + 1, minlat_idx : maxlat_idx + 1, minlon_idx : maxlon_idx + 1]
        return u, v
    ...
    def computeTaxis(self) -> Tuple[xr.DataArray, xr.DataArray]:
        ...
        param = dict(
            ...
            habitat_field=fh,
            use_SEAPODYM_global_mask=(not hf_cond),
            shallow_sea_to_ocean=ssto_cond,
            lat_min=fh.attrs["lat_min"],
            lat_max=fh.attrs["lat_max"],
            lon_min=fh.attrs["lon_min"],
            lon_max=fh.attrs["lon_max"],
        )
        ...
        return core.taxis(
            ...
            grad_lon,
            grad_lat,
            name=self.feeding_habitat.name,
        )

    def computeMortality(
        self, import_filepath: str = None, export_filepath: str = None, verbose: bool = False
    ) -> xr.DataArray:
        """
        ...
        if not hasattr(self.ikamoana_fields_structure, "selected_fisheries"):
            raise ValueError(
                "selected_fisheries can not be found. Make sure that you gave mortality and selected_fisheries tags."
            )
        ...
        return core.fishingMortality(
            ...
            effort_ds,
            params_fisheries,
            convertion_tab=ika_struct.selected_fisheries,
        )
    ...
    def computeIkamoanaFields(
        ...
        import_effort: str = None,
        export_effort: str = None,
        verbose: bool = False,
        ...
        domain = dict(
            time_start=time_start, time_end=time_end, lat_min=lat_min, lat_max=lat_max, lon_min=lon_min, lon_max=lon_max
        )

        if from_habitat is None:
            if evolve:
                self.computeEvolvingFeedingHabitat(cohort_start=cohort_start, cohort_end=cohort_end, **domain)
            else:
        ...
        if hasattr(self.ikamoana_fields_structure, "selected_fisheries"):
            mortality = self.computeMortality(import_effort, export_effort, verbose)
