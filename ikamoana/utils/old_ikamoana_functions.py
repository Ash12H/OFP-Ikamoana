...
def getGradient(field: Field, landmask: Field = None, shallow_sea_zero=False, time_period=None):
    """
    ...
    if time_period is not None:
        ...
        if field.grid.time[0] <= start_time:
            ...
            print(
                "Warning! Requested start time for field gradient calculation\
        ...
        if field.grid.time[-1] >= end_time:
            ...
            print(
                "Warning! Requested end time for field gradient calculation\
    ...
    return (
        Field(
            ...
            field.grid.lat,
            field.grid.depth,
            field.grid.time[range(start, end)],
            time_origin=field.grid.time_origin,
            interp_method=field.interp_method,
            allow_time_extrapolation=field.allow_time_extrapolation,
        ),
        Field(
            "d" + field.name + "_dy",
            dVdy,
...
def Create_Landmask(fields, lim=1e-45):
    ...
    Mask = Field("LandMask", mask, fields.H.grid.lon, fields.H.grid.lat, transpose=True, allow_time_extrapolation=True)
    Mask.interp_method = "nearest"
    return Mask


def Create_SEAPODYM_F_Field(
    Effort,
    length_classes,
    name="F",
    start_age=4,
    verbose=False,
    q=0.001032652877899101,
    selectivity_func=3,
    mu=52.56103941719986,
    sigma=8.614813906820441,
    r_asymp=0.2456242856428466,
    time_period=None,
    timestep=30 * 24 * 7 * 7,
):
    """
    ...
    if time_period is not None:
        ...
        if len(start) > 0:
            ...
            print(
                "Effort data does not exist for start time of %s! Using earliest timestamp %s"
    ...
    return Field(
        ...
        Effort.grid.lat,
        time=time,
        time_origin=Effort.grid.time_origin,
        interp_method="nearest",
    )


def Create_SEAPODYM_Diffusion_Field(
    H,
    length_classes,
    ...
    for t in range(len(tsteps)):
        ...
        if units == "nm_per_timestep":
            Dmax = (
                (np.power(GetLengthFromAge(age_class, length_classes) * (timestep / 1852), 2) / 4) * timestep / timestep
            )  # vmax = L for diffusion
        else:
        ...
        for x in range(H.grid.lon.size):
            for y in range(H.grid.lat.size):
                K[t, y, x] = (
                    sig_scale * sig_D * (1 - c_scale * c * np.power(Hdata[t, y, x], P)) * diffusion_scale
                    + diffusion_boost
                )
        if verbose:
            print("")
    ...
    return Field(
        ...
        H.grid.lon,
        H.grid.lat,
        time=H.grid.time[tsteps],
        time_origin=H.grid.time_origin,
        interp_method="nearest",
...
def Create_SEAPODYM_Taxis_Fields(
    dHdx: Field,
    dHdy: Field,
    length_classes: List[int],
    start_age=4,
    taxis_scale=1,
    units="m_per_s",
    vmax_a=2.225841100458143,
    vmax_b=0.8348850216641774,
    time_period=None,
    timestep=30 * 24 * 60 * 60,
) -> Tuple[Field, Field]:
    """
        ...
        if units == "nm_per_timestep":
            for x in range(dHdx.grid.lon.size):
                for y in range(dHdx.grid.lat.size):
                    Tx[t, y, x] = (
                        V_max(l, vmax_a, vmax_b)
                        * ((timestep) / 1852)
                        * xData[t, y, x]
                        * taxis_scale
                        * (1000 * 1.852 * 60 * math.cos(dHdx.lat[y] * math.pi / 180))
                        * 1
                        / timestep
                    )
                    Ty[t, y, x] = (
            ...
            for x in range(dHdx.grid.lon.size):
                for y in range(dHdx.grid.lat.size):
                    ...
                    Ty[t, y, x] = V_max(l, vmax_a, vmax_b) * yData[t, y, x] * taxis_scale * (250 * 1.852 * 15)
    ...
    return (
        Field(
            ...
            dHdx.grid.lon,
            dHdx.grid.lat,
            time=dHdx.grid.time[tsteps],
            time_origin=dHdx.grid.time_origin,
            interp_method=dHdx.interp_method,
...
def readSEAPODYM_parfile(file):
    variables = [
        ...
        "c_diff_fish",
        "MSS_species",
        "MSS_size_slope",  # movement params
        "Mp_mean_max",
        "Mp_mean_exp",
        "Ms_mean_max",
        "Ms_mean_slope",
    ...
    if file is not None:
        ...
        print(
            str(parfile.getElementsByTagName("mask_fishery_sp")[0].getElementsByTagName("skj")[0].firstChild.nodeValue)
        )
        F_toUse = [
            int(n)
            for n in str(
                parfile.getElementsByTagName("mask_fishery_sp")[0].getElementsByTagName("skj")[0].firstChild.nodeValue
            ).split()
        ]
        print(F_toUse)
        ...
        for f in SEAPODYM_Params["Fisheries"]:
            ...
            SEAPODYM_Params[f]["function_type"] = float(
                S[0].getElementsByTagName(f)[0].getElementsByTagName("function_type")[0].attributes["value"].value
            )
            SEAPODYM_Params[f]["mu"] = float(
                S[0].getElementsByTagName(f)[0].getElementsByTagName("length_threshold")[0].attributes["skj"].value
            )
            SEAPODYM_Params[f]["right_asymptote"] = float(
                S[0].getElementsByTagName(f)[0].getElementsByTagName("right_asymptote")[0].attributes["skj"].value
            )
        SEAPODYM_Params["Length_Classes"] = [
            float(l)
            for l in parfile.getElementsByTagName("length")[0].getElementsByTagName("skj")[0].childNodes[0].data.split()
        ]
        SEAPODYM_Params["SEAPODYM_dt"] = (
            int(parfile.getElementsByTagName("deltaT")[0].attributes["value"].value) * 24 * 60 * 60
        )
    else:  # Some defaults
        SEAPODYM_Params = {
            "sigma_species": 0.1769952864978924,
            "c_diff_fish": 0.662573993401526,
            "MSS_species": 2.225841100458143,
            "MSS_size_slope": 0.8348850216641774,
        }
...
def Field_from_DYM(filename, name=None, xlim=None, ylim=None, fromyear=None, frommonth=0, toyear=None, tomonth=0):
    ...
    class DymReader:
        ...
        typeNames = {"int32": "i", "uint32": "I", "int64": "q", "uint64": "Q", "float": "f", "double": "d", "char": "s"}
...
def getSEAPODYMarguments(run="SEAPODYM_2003"):
    ...
    if run == "SEAPODYM_2003":
        args.update(
            {
                "ForcingFiles": {
                    "U": "SEAPODYM_Forcing_Data/Latest/PHYSICAL/2003Run/2003run_PHYS_month*.nc",
        ...
        args.update({"Filestems": {"U": "m", "V": "m", "H": "d"}})
        args.update(
            {
                "Kernels": [
                    "Advection_C",
    ...
    if run == "DiffusionTest":
        ...
        args.update(
            {
                "ForcingFiles": {
                    "U": "DiffusionExample/DiffusionExampleCurrents.nc",
                    "V": "DiffusionExample/DiffusionExampleCurrents.nc",
                    "H": "DiffusionExample/DiffusionExampleHabitat.nc",
                }
            }
        )
        args.update({"ForcingVariables": {"U": "u", "V": "v", "H": "skipjack_habitat_index"}})
        args.update({"Filestems": {"U": "m", "V": "m", "H": "d"}})
        args.update({"Kernels": ["LagrangianDiffusion", "Move", "MoveOffLand"]})
    return args
