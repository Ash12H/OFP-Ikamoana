...
def equalCoords(data_array_1: xr.DataArray, data_array_2: xr.DataArray, verbose: bool = False) -> bool:
    """
...
def compareDims(data_array_1: xr.DataArray, data_array_2: xr.DataArray, dim: str, head: int = None):
    """
...
class HabitatDataStructure:
    def __init__(self, kargs: dict) -> None:
        ...
        self.topography = kargs["topography"]
    ...
    def _summaryToString(self) -> str:
        ...
        for name, number in zip(self.species_dictionary["life_stage"], self.species_dictionary["nb_cohort_life_stage"]):
            summary_str += (f"\t- There is(are) {number}\tcohort(s) in life stage {name}.\n")
    ...
    def findCoordIndexByValue(self, val: Union[float, List[float]], coord="lon", verbose: bool = False) -> List[int]:
        """
        Find the closest value(s) (`val`) in a specified coordinate
        (argument `coord`).
        """
        ...
        for v in val:
            ...
            if verbose:
                print(
                    "The %s coordinate closest to %s is index %d, which "
                    "is %s" % (coord, v, index, val_list[index].data)
                )
    ...
    def findCohortByLength(self, length: Union[float, List[float]], verbose: bool = False) -> List[int]:
        ...
        for l in length:
            ...
            if verbose:
                print(
                    "The cohort with the length closest to %f is cohort "
                    "number %d whose length is %f" % (l, index, length_list[index])
                )
    ...
    def findCohortByWeight(self, weight: Union[float, List[float]], verbose: bool = False) -> List[int]:
        ...
        for w in weight:
            ...
            if verbose:
                print(
                    "The cohort with the weight closest to %f is cohort "
                    "number %d whose weight is %f" % (w, index, weight_list[index])
                )
            cohort_number_list.append(index)
    ...
    def normalizeCoords(self):
        ...
        if time_reso == 30:  # SEAPODYM MONTHLY correspond to 30 days
            normalized_time_coords = np.array(time_coords, dtype="datetime64[M]")
        else:
            normalized_time_coords = np.array(time_coords, dtype="datetime64[D]")
        variable_update = {}
