...
def IkaDimMoveWithDiffusionReroll(particle, fieldset, time):
    ...
    while jump_loop < sections:
        ...
        jump_loop += 1
        if onland == 1:
            Rx = ParcelsRandom.uniform(-1.0, 1.0)
            Ry = ParcelsRandom.uniform(-1.0, 1.0)
...
def CalcLonLatScalers(particle, fieldset, time):
    ...
    particle.f_lat = 1 / 1000.0 / 1.852 / 60.0
...
def IkAdvectionRK4(particle, fieldset, time):
    ...
    uv_lon1 = particle.lon + u1 * 0.5 * particle.dt
    uv_lat1 = particle.lat + v1 * 0.5 * particle.dt

    (u2, v2) = fieldset.UV[time + 0.5 * particle.dt, particle.depth, uv_lat1, uv_lon1]
    uv_lon2 = particle.lon + u2 * 0.5 * particle.dt
    uv_lat2 = particle.lat + v2 * 0.5 * particle.dt

    (u3, v3) = fieldset.UV[time + 0.5 * particle.dt, particle.depth, uv_lat2, uv_lon2]
    ...
    particle.Ax = (u1 + 2 * u2 + 2 * u3 + u4) / 6.0 * particle.dt
    particle.Ay = (v1 + 2 * v2 + 2 * v3 + v4) / 6.0 * particle.dt
...
def TaxisRK4(particle, fieldset, time):
    ...
    t_lon1 = particle.lon + tx1 * 0.5
    t_lat1 = particle.lat + ty1 * 0.5

    tx2 = fieldset.Tx[time + 0.5 * particle.dt, particle.depth, t_lat1, t_lon1]
    ty2 = fieldset.Ty[time + 0.5 * particle.dt, particle.depth, t_lat1, t_lon1]
    t_lon2 = particle.lon + tx2 * 0.5
    t_lat2 = particle.lat + ty2 * 0.5

    tx3 = fieldset.Tx[time + 0.5 * particle.dt, particle.depth, t_lat2, t_lon2]
    ty3 = fieldset.Ty[time + 0.5 * particle.dt, particle.depth, t_lat2, t_lon2]
    ...
    particle.Tx = (tx1 + 2 * tx2 + 2 * tx3 + tx4) / 6.0 * particle.dt * particle.f_lon
    particle.Ty = (ty1 + 2 * ty2 + 2 * ty3 + ty4) / 6.0 * particle.dt * particle.f_lat
...
def NaturalMortality(particle, fieldset, time):
    Mnat = fieldset.MPmax * math.exp(-fieldset.MPexp * particle.age_class) + fieldset.MSmax * math.pow(
        particle.age_class, fieldset.MSslope
    )
