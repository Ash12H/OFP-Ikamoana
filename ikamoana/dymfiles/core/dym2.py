...
from . import dym, interpolation, netcdf
...
class DymFileHeader(dym.DymFileHeader):
    ...
    def readHeader(self):
        try:
            ...
            logging.exception("readHeader fails to read %s" % (e.fileName_))
            logging.exception(sys.exc_info())
            raise RuntimeError
        except:
            logging.exception("Header fails")
            logging.exception(sys.exc_info())
            raise RuntimeError
    ...
    def writeHeader(self):
        try:
            ...
            logging.exception("writeHeader fails to write file %s" % (self.fileName_))
            logging.exception(sys.exc_info())
            raise RuntimeError

        except Exception:
            logging.exception("writeHeader fails")
    ...
    def updateMask(self, newmask):
        try:
            ...
            logging.exception("in UpdateMask: input mask size mismatch")
            logging.exception("old shape : %d %d" % (oldshape))
            logging.exception("new shape : %d %d" % (newshape))
            raise RuntimeError
    ...
    def updateLevelValues(self, levelValues):
        try:
            ...
            logging.exception("new levels values size mismatch")
            raise RuntimeError
...
class DymFile(dym.DymFile):
    ...
    def binary_op(self, operator, inFile, outFile, force=True):
        ...
        if (
            self.header_.nLon_ != inDym.header_.nLon_
            and self.header_.nLat_ != inDym.header_.nLat_
            and self.header_.nLev_ != inDym.header_.nLev_
        ):
    ...
    def extractLevelsDateStr(self, firstDateStr, lastDateStr, outfile, dateFormat="%Y%m%d"):
        ...
        if minidx is not None and maxidx is not None:
            logging.info(
                "START DATE (index): %.6f %s (%d)"
                % (zlevels[minidx], dym.dates.floatToStr(zlevels[minidx], dateFormat=dateFormat), minidx + 1)
            )
            logging.info(
                "END   DATE (index): %.6f %s (%d)"
                % (zlevels[maxidx], dym.dates.floatToStr(zlevels[maxidx], dateFormat=dateFormat), maxidx + 1)
            )
        else:
    ...
    def extractSubdomain(self, latmin, latmax, lonmin, lonmax, outfile):
        ...
        if lonmin < xLonMin:
            ...
            # lonmin = lonmin + 360
            logging.error(f"{lonmin} is less than minimum longitude in file ({xLonMin}). Consider adding 360")
            raise RuntimeError
    ...
    def interpolate(self, maskFile, maskVarName, outFile, crop=0):
        ...
        if (Resolution - np.abs(Mylat[0] - Mylat[1])) / Resolution > 1e-3:
            ...
            logging.error(f"longitudinal resolution = {Resolution} ")
            logging.error(f"latitudinal  resolution = {Mylat[0] - Mylat[1]} ")
            raise RuntimeError
        ...
        logging.info("input  size and resolution: %4d x%4d %10g x%10g" % (in_nlon, in_nlat, in_dx, in_dy))
        logging.info("output size and resolution: %4d x%4d %10g x%10g" % (out_nlon, out_nlat, out_dx, out_dy))
        ...
        for t in range(self.header_.nLev_):
            ...
            outdata = interpolation.spatial(
                inveclon,
                np.flipud(inveclat),
                np.flipud(in_tmp),
                outveclon,
                outveclat,
                np.flipud(out_tmp_msk),
                fill_value=filval,
                mis_value=misval,
            )
...
def extractLevels(infile, firstDateStr, lastDateStr, output=None, dateFormat="%Y%m%d"):
    if not os.path.exists(infile):
        raise OSError("no such file %s" % infile)
