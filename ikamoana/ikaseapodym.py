...
from ikamoana.utils import coordsAccess, latitudeDirection, seapodymFieldConstructor
...
class IkaSeapodym(IkaSimulation):
    ...
    def __init__(self, filepath: str):
        ...
        super().__init__(parameter_file_dict.pop("run_name"), parameter_file_dict.pop("random_seed"))
    ...
    def _readConfigFile(self, filepath: str) -> dict:
        ...
        def readDomain(root: ET.Element, params: dict):
            ...
            params["spatial_limits"] = {
                "lonlim": (
                    np.float32(root.find("lon").find("min").text),
                    np.float32(root.find("lon").find("max").text),
                ),
                "latlim": (
                    np.float32(root.find("lat").find("min").text),
        ...
        def readCohortInfo(root: ET.Element, params: dict):
            ...
            if root.find("start_dynamic_file") is not None:
                params["start_dynamic_file"] = params["start_distribution"] + root.find("start_dynamic_file").text
                if "file_extension" in root.find("start_dynamic_file").attrib:
                    tmp = root.find("start_dynamic_file").attrib["file_extension"]
            ...
            if root.find("start_static_file") is not None:
                params["start_static_file"] = params["start_distribution"] + root.find("start_static_file").text
        ...
        def readForcing(root: ET.Element, params: dict):
            ...
            if files is not None:
                if "files_only" in files.attrib:
                    params["files_only"] = files.attrib["files_only"] in ["True", "true"]
                else:
                    params["files_only"] = False
                ...
                for elmt in files:
                    if "dataset" in elmt.attrib and elmt.attrib["dataset"] in ["True", "true"]:
                        forcing_dataset[elmt.tag] = os.path.join(home, elmt.text)
                    else:
                        forcing_dataarray[elmt.tag] = os.path.join(home, elmt.text)
            params["forcing_files"] = {"forcing_dataarray": forcing_dataarray, "forcing_dataset": forcing_dataset}
        ...
        def readAge(seapodym_filepath: str, start_length: float, params: dict):
            ...
            length_list = np.array([float(x) for x in root.find("length").find(sp_name).text.split()])
        ...
        def readMortality(ikamoana_root: ET.Element, params: dict):
            if (
                ikamoana_root.find("mortality") is not None
                ...
                if not os.path.isabs(effort_file_txt):
                    effort_file_txt = os.path.join(params["forcing_dir"], effort_file_txt)
                params["effort_file"] = f"{effort_file_txt}.nc"
    ...
    def loadFields(
        ...
        def generateFields():
            ...
            start = data_structure.findIndexByDatetime(self.ika_params["start_time"])[0]
                ...
                self.ika_params["start_time"] + np.timedelta64(self.ika_params["duration_time"] + 1, "s")
            )[0]
            ...
            import_effort = self.ika_params["effort_file"] if self.ika_params.pop("import_effort", False) else None
            export_effort = self.ika_params["effort_file"] if self.ika_params.pop("export_effort", False) else None
            ...
            return generator.computeIkamoanaFields(
                ...
                lat_min=latlims[1],
                lat_max=latlims[0],
                import_effort=import_effort,
                export_effort=export_effort,
            )
    ...
    def _fromCellToStartField(self) -> xr.DataArray:
        ...
        coords = {"time": [self.ika_params["start_time"]], "lat": grid_lat, "lon": grid_lon}
        dimensions = ("time", "lat", "lon")

        start_dist = xr.DataArray(name="start_dist", data=start, coords=coords, dims=dimensions)
        self.start_coords = start_dist.coords  # For density calculation later
        start_dist = parcels.Field.from_xarray(
            start_dist, name="start_dist", dimensions={d: d for d in dimensions}, interp_method="nearest"
        )
    ...
    def _fromStartFieldToCoordinates(
        ...
        inds = np.random.choice(data.size, particles_number, replace=True, p=p[0] / np.sum(p))
    ...
    def _rescaleFieldWithUCoordinates(self, field):
        ...
        if "time" in field.indexes:
            return field.isel(time=0, lat=slice(minlat_idx, maxlat_idx + 1), lon=slice(minlon_idx, maxlon_idx + 1))
        else:
            return field.isel(lat=slice(minlat_idx, maxlat_idx + 1), lon=slice(minlon_idx, maxlon_idx + 1))

    def initializeParticleSet(
        self,
        particles_longitude: Union[list, np.ndarray] = None,
        ...
        def initializeWithDynamicFile():
            ...
            if start_age < 0:
                raise ValueError(
                    "start_age must be greater than 0. Initial distribution will use previous age cohort file."
                )

            start_field = seapodymFieldConstructor(
                f"{file_prefix}{start_age}.{file_extension}",
                dym_varname=f"{file_prefix}{start_age}",
            )
            start_field = latitudeDirection(start_field, south_to_north=True)
        ...
        if particles_longitude is None and particles_latitude is None:
            ...
            if condition_multiple and method not in ["start_cell", "start_static_file", "start_dynamic_file"]:
                raise ValueError(
                    "Both start_cell, start_filestem and start_dynamic_file are "
                    "defined in configuration file. Specify which one you want "
                    "to use in method attribut."
            ...
            if (condition_multiple and method == "start_cell") or (condition_cell and not condition_multiple):
                particles_longitude, particles_latitude = initializeWithCell()
            elif (condition_multiple and method == "start_static_file") or (
                condition_static_file and not condition_multiple
            ):
                particles_longitude, particles_latitude = initializeWithStaticFile()
            elif (condition_multiple and method == "start_dynamic_file") or (
                condition_dynamic_file and not condition_multiple
            ):
        ...
        variables = {
            "age_class": [self.ika_params["start_age"]] * nb_particles,
            "age": [self.ika_params["start_age"] * self.ika_params["delta_time_seapodym"]] * nb_particles,
            **particles_variables,
        }

        super().initializeParticleSet(
            particles_longitude, particles_latitude, particles_class, particles_starting_time, variables
        )

    def runKernels(
        self,
        kernels: Union[KernelType, Dict[str, KernelType]] = None,
        ...
        if kernels is None:
            ...
            for k in self.ika_params["kernels"]:
                if k in behaviours.AllKernels:
                    ...
                    raise ValueError(f"{k} kernel is not defined by " "behaviours.AllKernels.")

        if recovery is None:
            recovery = {parcels.ErrorCode.ErrorOutOfBounds: behaviours.KillFish}

        super().runKernels(
            kernels=behaviours_dict,
            ...
            output_delta_time=output_delta_time,
            verbose=verbose,
            *kargs,
        )
