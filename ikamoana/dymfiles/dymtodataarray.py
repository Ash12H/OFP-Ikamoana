...
from .core import dym2, utilities


def dym2ToDataArray(infilepath: str, varname: str, attributs: dict = None) -> xr.DataArray:
    """
    ...
    returned_da = xr.DataArray(
        data=np.array(outdata_list),
        name=varname,
        dims=("time", "lat", "lon"),
        coords=(
            ("time", np.array(datestr_list, dtype="datetime64[D]"), dict_time),
            ("lat", veclat, dict_lat),
            ("lon", veclon, dict_lon),
        ),
        attrs=attributs,
