...
def seapodymFieldConstructor(filepath: str, dym_varname: str = None, dym_attributs: dict = None) -> xr.DataArray:
    """
    ...
    if exists(filepath):
        ...
        if filepath.lower().endswith(".dym"):
            ...
            return df.dym2ToDataArray(infilepath=filepath, varname=dym_varname, attributs=dym_attributs)
    else:
        raise ValueError(f"No such file : {filepath}")


def tagReading(
    root: ET.Element, tags: Union[str, List[str]], default: Union[str, int, float] = None, attribute: str = None
) -> Union[int, float, str]:
    """
