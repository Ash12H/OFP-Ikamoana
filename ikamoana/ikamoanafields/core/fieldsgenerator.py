...
def landmask(
    ...
    lat_min, lat_max, lon_min, lon_max = controlArguments(habitat_field, lat_min, lat_max, lon_min, lon_max)
    ...
    if use_SEAPODYM_global_mask:
        ...
        # ocean.
        mask_L1 = data_structure.global_mask["mask_L1"][0, lat_min:lat_max, lon_min:lon_max]
        mask_L3 = data_structure.global_mask["mask_L3"][0, lat_min:lat_max, lon_min:lon_max]
        ...
        coords = {
            "lat": data_structure.coords["lat"][lat_min:lat_max],
            "lon": data_structure.coords["lon"][lon_min:lon_max],
        }

    else:
        if habitat_field is None:
            raise ValueError("You must specify a habitat_field argument if use_SEAPODYM_global_mask is False.")
        habitat_f = habitat_field[0, :, :]
        lmeso_f = data_structure.variables_dictionary["forage_lmeso"][0, lat_min:lat_max, lon_min:lon_max]

        if habitat_f.shape != lmeso_f.shape:
            raise ValueError(
                f"Habitat {habitat_f.shape} and forage_lmeso {lmeso_f.shape} must have the same dimension."
            )
        ...
        coords = {"lat": habitat_field.coords["lat"], "lon": habitat_field.coords["lon"]}

    if field_output:
        if habitat_field is None:
            raise ValueError(
                "If field_output is True you must passe a "
                "habitat_field. Otherwise the time coordinate "
                "length can't be calculated."
            )
        landmask = np.tile(landmask[np.newaxis], (habitat_field.time.size, 1, 1))
    ...
    landmask = latitudeDirection(
        xr.DataArray(data=landmask, name="landmask", coords=coords, dims=dimensions), south_to_north=True
    )
    return landmask


def gradient(field: xr.DataArray, landmask: xr.DataArray, name: str = None) -> Tuple[xr.DataArray, xr.DataArray]:
    """
    ...
    """
    if (field.lat.size != landmask.lat.size) or (field.lon.size != landmask.lon.size):
        ...
        for lon in range(1, nlon - 1):
            for lat in range(1, nlat - 1):
                if landmask[lat, lon] < 1:
                    ...
                    dVdlat[t, lat, lon] = 0.0
                    dVdlon[t, lat, lon] = 0.0
...
def _diffusionCorrection(diffusion, dHdx, dHdy, dx, dy, current_u=None, current_v=None, vertical_movement=True):
    """
    ...
    if vertical_movement:
        if current_u is None or current_v is None:
            raise ValueError("If vertical_movement is True, you must specify currents fields (current_u, current_v)")
...
def diffusion(
    ...
    current_u: xr.DataArray = None,
    current_v: xr.DataArray = None,
    ...
    for t in range(habitat.time.size):
        ...
        diffusion = (
            d_max
            * (1 - ika_structure.c_scale * ika_structure.c * np.power(habitat_data[t, :, :], ika_structure.P))
        ...
        diffusion_x[t, :, :], diffusion_y[t, :, :] = _diffusionCorrection(
            ...
            current_u[t, :, :],
            current_v[t, :, :],
            ika_structure.vertical_movement,
        )
    ...
    return (
        xr.DataArray(
            data=diffusion_x,
            name="Kx_" + (habitat.name if name is None else name),
            coords=habitat.coords,
            dims=("time", "lat", "lon"),
            attrs=diffusion_attrs,
        ),
        xr.DataArray(
            data=diffusion_y,
            name="Ky_" + (habitat.name if name is None else name),
            coords=habitat.coords,
            dims=("time", "lat", "lon"),
            attrs=diffusion_attrs,
        ),
...
def fishingMortality(
    ...
    def selectivity(function_type, sigma, mu, r_asymp=None):
        if function_type == 3:
            return lambda length: selectivityAsymmetricGaussian(length, mu=mu, r_asymp=r_asymp, sigma_sq=sigma**2)
        elif function_type == 2:
            return lambda length: selectivitySigmoid(length, sigma=sigma, mu=mu)
        else:
            return lambda length: selectivityLimitOne(length, sigma=sigma)
    ...
    for p_name, params in fisheries_parameters.items():
        ...
        if f_name in effort_ds:
            ...
            selectivity_fun = selectivity(
                ...
                r_asymp=(params["right_asymptote"] if "right_asymptote" in params else None),
            )

            if evolving:
                c_nb = fh_structure.cohorts_number
                tmp = np.arange(start_age, c_nb)
                age = np.concatenate((tmp, np.repeat(c_nb - 1, effort_ds.time.data.size - tmp.size)))
            else:
                age = start_age
            ...
            fishing_mortality[f_name] = xr.DataArray(
                f_data, coords=effort_ds[f_name].coords, attrs=effort_ds[f_name].attrs
            )
